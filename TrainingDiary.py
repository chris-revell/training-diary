...
ax[1].set_ylim([120,195])
ax[1].set_ylabel("Heart Rate /bpm")

speeddataarray = data["speed"].values
#speeddataarray = 500/speeddataarray-120
means,bin_edges,binnumber = binned_statistic(data.time.values/np.timedelta64(1, 'm'),speeddataarray,bins=len(data.time.values)/2)
ax[2].plot(bin_edges[:-1]+(bin_edges[1]-bin_edges[0])/2,means)
ax[2].set_xlabel("Time")
ax[2].set_ylabel("Pace /(s/500m)")
ax[2].set_ylim([4.1,4.8])
ax[2].set_xlim([0,8.2])
ticks = ax[2].get_yticks()
ax[2].set_yticklabels((500/ticks).astype('int')-60)
